...
const document = {
  ...
  extractedText:
    "The State represented that a GPS tracker was installed on the vehicle.",
  embedding: null,
  status: "completed",
  summary: null,
  createdAt: new Date(),
    ...
    sourceAnchors: [
      {
        ...
        quote: "GPS tracker was installed",
      },
    ],
...
describe("leverageEngine guardrails", () => {
  it("parses structured findings from agent JSON", () => {
    ...
    expect(output.findings[0].sourceAnchors[0].fileName).toBe(
      "tracker-motion.pdf"
    );
  });
        ...
        sourceAnchors: [
          {
            documentId: document.id,
            fileName: document.fileName,
            quote: "a helicopter followed him",
          },
        ],
      }),
      [document]
  ...
  it("requires QC for high-risk findings even above the threshold", () => {
    const gated = applyRiskBasedQcGate(
      finding({ confidence: 99, liabilityVector: "Monell municipal liability" })
    );
  ...
  it("requires QC for v2 skeptical inference labels", () => {
    const gated = applyRiskBasedQcGate(
      finding({ confidence: 99, findingType: "suspicious_absence" })
    );

    expect(gated.qcStatus).toBe("pending");
  });

  it("adds FILE_WRIT labels when a mandamus finding has all writ elements", () => {
    const output = parseStructuredAgentOutput(
      { id: "mandamus_writ_architect", name: "Mandamus Writ Architect" },
      JSON.stringify({
        summary: "One mandamus finding",
        findings: [
          finding({
            title: "Pending motion requires a ruling",
            liabilityVector: "Mandamus / due process",
            remedyPath: "mandamus",
            missingRecords: [],
            summary:
              "The record supports a clear duty to rule and a narrow command to compel a ruling on the pending motion. Petitioner is beneficially interested because the unresolved motion directly affects current review.",
            nextAction:
              "Appeal is not adequate because meaningful review is impossible without a ruling.",
          }),
        ],
      }),
      [document]
    ...
    expect(output.findings[0].nextAction).toContain("FILE_WRIT:");
    expect(output.findings[0].qcStatus).toBe("pending");
    expect(output.findings[0].qcReason).toContain("deterministic writ gate");
  });

  it("keeps incomplete mandamus findings in the records-first lane", () => {
    const output = parseStructuredAgentOutput(
      { id: "mandamus_writ_architect", name: "Mandamus Writ Architect" },
      JSON.stringify({
        summary: "One incomplete mandamus finding",
        findings: [
          finding({
            title: "Pending ruling may support writ relief",
            liabilityVector: "Mandamus / due process",
            remedyPath: "mandamus",
            missingRecords: [],
            nextAction:
              "Verify the appendix and explain why ordinary appeal is inadequate.",
          }),
        ],
      }),
      [document]
    ...
    expect(output.findings[0].nextAction).toContain("DEMAND_RECORDS_FIRST:");
    expect(output.findings[0].qcStatus).toBe("pending");
  });
  ...
  it("fallback QC blocks record-supported claims with no anchors", () => {
    const audit = fallbackQcAuditForFinding(
      finding({ sourceAnchors: [] }),
      new Error("auditor unavailable")
    );
